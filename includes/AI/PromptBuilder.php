...
class PromptBuilder {
	 ...
	 * @param string               $patterns_section        Formatted pattern list; omitted when empty.
	 */
	public static function build( array $catalog, string $editor_content = '', string $additional_instructions = '', string $patterns_section = '' ): string {
		...
		if ( '' !== $patterns_section ) {
			$sections[] = "## Available Patterns\n\nThe site has pre-designed block patterns. When a pattern matches the user's request, prefer it over assembling an equivalent layout from individual blocks. Patterns are professionally designed and include spacing, typography, and color decisions that cannot be reproduced by assembling raw blocks.\n\nTo use a pattern, return: {\"pattern\": \"pattern-namespace/pattern-name\"}\n\nOnly use patterns from this list:\n\n" . $patterns_section;
		}

		$pattern_format = '' !== $patterns_section
			? "\n\nAlternatively, when a registered pattern closely matches the request, return a pattern reference instead of blocks:\n{\"pattern\": \"pattern-namespace/pattern-name\"}\nUse only names from the Available Patterns list above."
			: '';
